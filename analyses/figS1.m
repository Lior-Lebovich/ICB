...
for i = 1:length(bias_imp_pos_unique)
    ...
    if i == length(bias_imp_pos_unique)
        d = plot( bias_imp_pos_unique(i,1), bias_imp_pos_unique(i,2), ...
            'Marker','o','MarkerSize',4+2*(mSize-1), ...
            'MarkerFaceColor', [0,0,0],'Color',[1 1 1]); hold on; 
    else
        plot( bias_imp_pos_unique(i,1), bias_imp_pos_unique(i,2), ...
            'Marker','o','MarkerSize',4+2*(mSize-1), ...
            'MarkerFaceColor', [0,0,0],'Color',[1 1 1]); hold on; 
    end
end
...
legend( [d,h], ['data: rho = ' num2str(rho) ', p = ' num2str(pVal)], ...
