...
!	Questions to david.hansel@parisdescartes.fr
	...
	integer i1,i2,i3,npref1,npref2,narch,iarch,nl,il
...
integer, allocatable :: nspike(:),nspiki(:)
	...
	real*8 inputcuee1,inputcuei1,dl
...
allocate (nspike(n),nspiki(n))
	...
	open(2,file='psychometric-upload.ini')
	read(2,*) semarch 
	read(2,*) seminit
	read(2,*) itmax
	read(2,*) dtf
...
!	open(4,file='conect-psycho-'//nombfile0)
 	open(96,file='bias-psycho-'//nombfile0)
 	open(98,file='choice-psycho-'//nombfile0)
!	open(97,file='orderparameter-psycho-'//nombfile0)
!	open(99,file='numrec-psycho-'//nombfile0)
!	open(100,file='numrecsparse-psycho-'//nombfile0)
!
	kkee0=kkee
	kkei0=kkei
	kkie0=kkie
	...
	open(12,file='pr-psycho-'//nombfile0)
	...
	write(12,*) 'inputcuee1,inputcuei1=',inputcuee1,inputcuei1
...
do il=0,nl
	avtimedec=0.d0
	nact1=0
!	dl=0.1*float(il-nl/2)
	write(12,*) 'dl=',dl
do i=1,ne/2
inputcuee(i)=dsqrt(kkee)*inputcuee0+dl*inputcuee1
inputcuee(i+ne/2)=dsqrt(kkee)*inputcuee0-dl*inputcuee1
end do
do i=1,ni/2
inputcuei(i)=dsqrt(kkee)*inputcuei0+dl*inputcuei1
inputcuei(i+ni/2)=dsqrt(kkee)*inputcuei0-dl*inputcuei1
end do
...
!	open(3,file='ux-psycho-'//r(itrial))
!       open(26,file='spike-psycho-'//r(itrial))
!       open(27,file='spiki-psycho-'//r(itrial))
!       open(31,file='fire-psycho-'//r(itrial))
!       open(32,file='firi-psycho-'//r(itrial))
                nspike=0
                nspiki=0
		stim=0.
...
!	write(97,*) sngl(epsilon),sngl(dabs(avfire1-avfire2)/(avfire1+avfire2)),sngl((avfire1+avfire2)/2.d0*100.),sngl(dabs(avfiri1-avfiri2)/(avfiri1+avfiri2)),sngl((avfiri1+avfiri2)/2.d0*100.)
	...
	write(96,*) dl,ntrial,sngl(float(nact1)/float(ntrial)),sngl(avtimedec/ntrial)
	end do !il
	end do !iarch
	end 
